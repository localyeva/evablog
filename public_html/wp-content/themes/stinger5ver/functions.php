...
function redirect_url() {
    if (get_site_url() == DT_CUR_URL):
        header('location:' . EVOLABLE_ASIA . '/eva-blog');
    endif;
    exit();
}
?>
...
function get_lastest_news() {
    ...
    $sql = 'select p.*
                from wp_terms t
                    join wp_term_taxonomy ta on ta.term_id = t.term_id
                    join wp_term_relationships r on r.term_taxonomy_id = ta.term_taxonomy_id
    ...
    $results = $wpdb->get_results($sql, OBJECT);
    $res = $results[0];
    $post_thumnail = get_thumnail($res->ID);
